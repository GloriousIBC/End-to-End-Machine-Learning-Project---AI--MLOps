...
class ZipDataIngestor(DataIngestor):
    def ingest(self, file_path: str) -> pd.DataFrame:
        ...
        # Find the extracted CSV file (assuming there is one CSV file in the zip)
        extracted_files = os.listdir("extracted_data")
        csv_files = [f for f in extracted_files if f.endswith(".csv")]

        if len(csv_files) == 0:
            raise FileNotFoundError("No CSV file found in the extarcted data.")
        if len(csv_files) > 1:
...
if __name__ == "__main__":
    ...
    file_path = "/Users/DELL/Desktop/End-to-End Machine Learning Project – AI, MLOps/data/archive.zip"
